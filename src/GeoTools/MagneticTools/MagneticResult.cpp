...
void MagneticResult::calculateMagneticField() {
    ...
    for (int n = 1; n < MagneticUtil::WMM_EXPANSION; n++)
    {
        ...
        for (int m = 0; m <= n; m++)
        {
            ...
            std::shared_ptr<MagneticModelCoefficients> wmmCoefficients = model->getCoeffs(n, m)->getPointOnDate(t0, t); // Equation 9
...
MagneticResult::MagneticResult(const std::shared_ptr<const MagneticModel>& model, const GeoPoint &point,
...
const MagneticFieldElements &MagneticResult::getMainFieldElements() const {
